...
class SearchApp extends MallbaseApp
{
	...
	function zhaopin(){
		...
		if($_POST){
			...
			if($_POST['cate_id'] == '全部'){
				...
				$query="select * from ecm_zhaopin where status=0 and title like '%$key%'"; //定义sql
				
			}else{
				
				$query="select * from ecm_zhaopin where status=0 and hangye='$cate_id' and title like '%$key%'"; //定义sql
			}
			$this->assign('key', $key);
			...
			if(empty($_GET['cate_id']) || !isset($_GET['cate_id'])){
				$query="select * from ecm_zhaopin where status=0"; //定义sql
			}else{
		...
		if($result != false){
			while($rows=@mysql_fetch_array($result)){
				...
				$con .="		<li><i><img src='/themes/mall/jiaju/styles/style/zp/images/index_07.jpg'></i>{$rows['title']}</li>";
				$con .="		<li>{$rows['gname']}</li>";
				$con .="		<li>{$rows['s_province']}{$rows['s_city']}{$rows['s_county']}{$rows['xiangxi']}</li>";
				$con .="		<li>{$rows['addtime']}</li>";
	...
	function weizhi(){
		...
		$query="SELECT *FROM `ecm_goods` where status='已审核' and goods_id='$goods_id'"; //定义sql
		...
		$store_query="SELECT *FROM `ecm_store_user` where (cate_id='$cate_id' or cate_id='$cate_id_1' or cate_id='$cate_id_2' or cate_id='$cate_id_3' or cate_id='$cate_id_4') and status='0'"; //定义sql
