...
<article class="page-container">
	<form action="/duanxin/index.php/Admin/Index/typeadd" method="post" class="form form-horizontal" id="form-type-up">
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>号码类型：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo ($typename); ?>" placeholder="号码类型" id="typename" name="typename" datatype="*4-16" nullmsg="号码类型不能为空">
			</div>
		</div>
...
function tijiao(){
		...
		cache: true,
		type: "POST",
		url:"/duanxin/index.php/Admin/Index/typeupdate",
		...
		async: false,
		  ...
		  success: function(data) {
			...
			if(data == 'no'){
			//alert("类型值不能为空，请输入！");
			layer.msg('类型值不能为空，请输入！',{icon:1,time:1000});
			}
			if(data == 'has'){
			//alert('此类型已经存在请重新输入！');
			layer.msg('此类型已经存在请重新输入！',{icon:1,time:1000});
			}
		  }
	  });
		return false;
