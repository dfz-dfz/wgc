...
<article class="page-container">
	<form action="/duanxin/index.php/Admin/Index/typeadd" method="post" class="form form-horizontal" id="form-type-add">
		<div class="row cl">
		...
		<div class="row cl">
			<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-3">
				<button type="submit" class="btn btn-success radius" onclick="return tijiao()"><i class="icon-ok"></i> 确定</button>
			</div>
...
function tijiao(){
		...
		cache: true,
		type: "POST",
		url:"/duanxin/index.php/Admin/Index/typeadd",
		...
		async: false,
		  ...
		  success: function(data) {
			...
			if(data == 'err'){alert("添加失败");}
			if(data == 'no'){alert("类型值不能为空，请输入！");}
			if(data == 'has'){alert('此类型已经存在请重新输入！');}
		  }
	  });
		return false;
	}
