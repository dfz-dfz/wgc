		...
		<div>
			...
			<div class="page-content" >
				<div class="page-header" style="width:100%;height:45px;padding-top:12px;">
					<form action="/manage/index.php/Admin/Mobile/mobilelist" method="get" >
						手机号码：<input  type="text" name="mobe" placeholder="手机号码" value="<?php echo ($_GET['mobe']); ?>" />
								&nbsp;&nbsp;
						时间：
						...
						<button type="submit" class="btn btn-xs btn-primary">搜索</button>　　　<span style="background-color:#428BCA;font-size:17px;margin-bottom:0px;"><a href="javascript:;" style="color:#FFFFFF;" onclick="mobile_add('添加','/manage/index.php/Admin/Mobile/mobileadd','800')">添加</a></span>
					</form>
				</div><!-- /.page-header -->
				<div class="row">
					<form action="<?php echo U('Admin/Mobile/mobiledel');?>" method="post" id="formids">
						...
						<tbody>
							<?php if(is_array($list)): foreach($list as $key=>$v): ?><tr>
								<td class="center" width="80px">
									<label>
										<input type="checkbox"  name="mobeid[]" value="<?php echo ($v["mobeid"]); ?>"/>
										<!--<span class="lbl"></span>-->
									</label>
								</td>

								<th><?php echo ($v["mobeid"]); ?></th>
								<th><?php echo ($v["mobe"]); ?></th>
								<th><?php echo ($v["name"]); ?></th>
								<th><?php echo ($v["typeid"]); ?>(<?php echo ($v["typename"]); ?>)</th>
								<th><?php echo ($v["area"]); ?></th>
								...
								<td width="100px">
									<div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">
										...
										<a class="blue" onClick="mobe_edit('编辑','<?php echo U('Admin/Mobile/mobileupdate',array('mobeid'=>$v['mobeid']));?>','800')" href="javascript:;">
											<!--<i class="icon-edit bigger-130"></i>-->编辑
										</a>

										<a class="red" onclick="return mobe_del('<?php echo U('Admin/Mobile/mobiledel',array('mobeid'=>$v['mobeid']));?>')" href="javascript:;">
							...
							<tr>
								<td colspan="2">
									<input type="button" value="删除" onclick="return mode_delmall('<?php echo U('Admin/Mobile/mobiledel');?>')" style="background-color:#F70808;" />
								</td>
								<td colspan="7">
...
/*类型-编辑*/
function mobe_edit(title,url,w,h){
	layer_show(title,url,w,h);
}
/*类型-删除*/
function mobe_del(lujing){
